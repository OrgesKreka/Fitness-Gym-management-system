...
<script>
        ...
        if( jQuery( '#kerko' ).hasClass( 'has-error' ) )
        {
          ...
          { ///Therritja ajax per te kontrolluar vleren ne databaze
            jQuery.ajax(
              {
                method: "POST",
                url: "http://localhost/palestra/search_includes/autocomplete.php",
                data: data,
                success: function( pergjigje )
                {
                      ...
                      if( pergjigjet[ 0 ] == '-1')
                      {
                          ...
                          // option.value = "Vlera e kerkuar nuk gjendet!";
    ...
    function shfaqFormeRegjistrimi( id )
    {
        if( id == 1 )
          ...
          jQuery( '#modaRegjistrimi2' ).modal( 'show' );
...
<?php ///Nese kodi i regjistrimit eshte i gabuar...
